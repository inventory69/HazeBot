...
def test_load_performance(session_counts: list):
    ...
    for count in session_counts:
        ...
        try:
            # Generate sessions
            print(f"  ├─ Generating fake sessions...", end=" ", flush=True)
            start = time.time()
            ...
            print(f"  ├─ Writing to disk...", end=" ", flush=True)
            start = time.time()
            data = {
                "sessions": sessions,
                "daily_stats": {},
            ...
            print(f"  ├─ Loading analytics...", end=" ", flush=True)
            start = time.time()
            ...
            # Test reprocessing
            print(f"  ├─ Reprocessing aggregations...", end=" ", flush=True)
            start = time.time()
            ...
            print(f"  ├─ Export (uncached)...", end=" ", flush=True)
            ...
            # Test export (cached)
            print(f"  ├─ Export (cached)...", end=" ", flush=True)
            start = time.time()
            export_data = analytics.get_export_data(days=7)
            ...
            print(f"  ├─ Summary (uncached)...", end=" ", flush=True)
            analytics.cache.invalidate()
            start = time.time()
            ...
            # Test summary (cached)
            print(f"  ├─ Summary (cached)...", end=" ", flush=True)
            start = time.time()
            summary = analytics.get_summary_stats()
...
def print_summary(load_results: dict, batch_results: dict):
    ...
    for count, results in load_results.items():
        print(f"{count:>10,} {results['load_time']:>8.2f}s {results['reprocess_time']:>10.2f}s "
    ...
    print(f"  • Cache speedup: {max_result['export_speedup']:.1f}x (export), "
