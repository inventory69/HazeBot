...
async def get_monitoring_status():
    ...
    if not UPTIME_KUMA_URL:
        ...
        return jsonify(
            {"error": "Uptime Kuma not configured", "message": "UPTIME_KUMA_URL environment variable not set"}
        ), 503
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(UPTIME_KUMA_URL, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status != 200:
                    ...
                    return jsonify(
                        {"error": f"Uptime Kuma returned status {resp.status}", "status_code": resp.status}
                    ), 502
        ...
        return jsonify({"error": "Network error connecting to Uptime Kuma", "details": str(e)}), 502
...
def calculate_overall_status(monitors: list) -> str:
    ...
    pending_with_low_uptime = sum(1 for m in monitors if m.get("status") == "pending" and m.get("uptime", 100) < 98)
