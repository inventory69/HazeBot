...
@hazehub_cogs_bp.route("/api/cogs", methods=["GET"])
def get_cogs():
    ...
    try:
        ...
        # Cog descriptions and metadata (abbreviated for space)
        cog_metadata = {
            "APIServer": {"description": "Manages the Flask REST API server", "category": "core"},
            "CogManager": {"description": "Dynamic cog loading/unloading system", "category": "core"},
            "Changelog": {"description": "Changelog notifications", "category": "notifications"},
            "DailyMeme": {"description": "Automated daily meme posting", "category": "content"},
            "DiscordLogging": {"description": "Logs bot events to Discord", "category": "monitoring"},
            "GamingHub": {"description": "Community gaming features", "category": "community"},
            "Leaderboard": {"description": "Server activity leaderboard", "category": "community"},
            "MemeGenerator": {"description": "Create custom memes", "category": "content"},
            "ModPerks": {"description": "Moderator utilities", "category": "moderation"},
            "Preferences": {"description": "User preference management", "category": "user"},
            "Presence": {"description": "Bot status rotation", "category": "core"},
            "Profile": {"description": "User profile viewing", "category": "user"},
            "RocketLeague": {"description": "Rocket League rank tracking", "category": "gaming"},
            "RoleInfo": {"description": "Server role information", "category": "info"},
            "ServerGuide": {"description": "Interactive server guide", "category": "info"},
            "SupportButtons": {"description": "Persistent support buttons", "category": "support"},
            "TicketSystem": {"description": "Ticket management system", "category": "support"},
            "TwitchAlerts": {"description": "Twitch stream notifications", "category": "notifications"},
            "Voice": {"description": "Voice channel utilities", "category": "utility"},
            "Warframe": {"description": "Warframe game integration", "category": "gaming"},
            "WelcomeSystem": {"description": "Member welcome system", "category": "community"},
        }
        ...
        for file_name, class_name in all_cogs.items():
            is_loaded = class_name in loaded_cogs
            is_disabled = class_name in disabled_cogs
            metadata = cog_metadata.get(class_name, {})
        ...
        cogs_list.sort(key=lambda c: (not (c["status"] == "loaded"), c["name"].lower()))
        ...
        return jsonify(
            {
                ...
                "disabled_count": len([c for c in cogs_list if c["disabled"]]),
            }
        )
...
@hazehub_cogs_bp.route("/api/cogs/<cog_name>/load", methods=["POST"])
...
@hazehub_cogs_bp.route("/api/cogs/<cog_name>/unload", methods=["POST"])
...
@hazehub_cogs_bp.route("/api/cogs/<cog_name>/reload", methods=["POST"])
def reload_cog(cog_name):
    ...
    try:
        ...
        # Reload the cog - longer timeout for APIServer
...
@hazehub_cogs_bp.route("/api/cogs/<cog_name>/logs", methods=["GET"])
