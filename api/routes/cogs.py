...
@cogs_bp.route("/api/cogs/<cog_name>/load", methods=["POST"])
@token_required
@require_permission("all")
...
@cogs_bp.route("/api/cogs/<cog_name>/unload", methods=["POST"])
@token_required
@require_permission("all")
...
@cogs_bp.route("/api/cogs/<cog_name>/reload", methods=["POST"])
@token_required
@require_permission("all")
...
@cogs_bp.route("/api/cogs/<cog_name>/logs", methods=["GET"])
@token_required
@require_permission("all")
def get_cog_logs(cog_name):
    ...
    try:
        ...
        if not os.path.exists(log_file):
            alternative_paths = [
                "Logs/HazeBot.log",
                os.path.join(Path(__file__).parent.parent.parent, "Logs", "HazeBot.log"),
        ...
        logs = []
        for log_line in filtered_logs[-50:]:
            ...
            if len(parts) >= 4:
                ...
                level = parts[2]
                message = parts[3]
                logs.append(f"{timestamp} {time} {level} {message}")
