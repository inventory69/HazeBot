...
from Config import PINK, ADMIN_ROLE_ID, MODERATOR_ROLE_ID, MOD_DATA_FILE, CHANGELOG_ROLE_ID
...
def add_mod_action(action, user_id, reason, mod):
...
class KickModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            add_mod_action("kicks", self.member.id, reason_text, interaction.user)
            await interaction.response.send_message(f"👢 Kicked {self.member.mention}. Reason: {reason_text}", ephemeral=True)
...
class BanModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            add_mod_action("bans", self.member.id, reason_text, interaction.user)
            await interaction.response.send_message(f"🔨 Banned {self.member.mention}. Reason: {reason_text}", ephemeral=True)
...
class MuteModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            add_mod_action("mutes", self.member.id, reason_text, interaction.user)
            await interaction.response.send_message(f"🔇 Muted {self.member.mention}. Reason: {reason_text}", ephemeral=True)
...
class ModMainView(discord.ui.View):
    ...
    async def mod_overview(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Verwende handle_modoverview
    ...
    async def opt_ins(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Verwende handle_optins
        cog = self.bot.get_cog("ModPanel")
        if cog:
...
class UserSelectForDetails(discord.ui.Select):
    def __init__(self, members):
        options = [
            discord.SelectOption(label=f"{m.display_name}#{m.discriminator}", value=str(m.id))
            for m in members[:25]  # Limitiere auf 25 Optionen
        ]
        super().__init__(
...
class ModPanel(commands.Cog):
    ...
    async def handle_modpanel(self, ctx_or_interaction):
        if not is_mod_or_admin(ctx_or_interaction.author if hasattr(ctx_or_interaction, 'author') else ctx_or_interaction.user):
            ...
            return
        embed = create_modpanel_embed(self.bot.user)
        members = [m for m in (ctx_or_interaction.guild if hasattr(ctx_or_interaction, 'guild') else ctx_or_interaction.guild) .members if not m.bot]
        if not members:
        ...
        modpanel_logger.info(f"Mod panel opened by {ctx_or_interaction.author if hasattr(ctx_or_interaction, 'author') else ctx_or_interaction.user}")
    ...
    async def handle_mod(self, ctx_or_interaction):
        if not is_mod_or_admin(ctx_or_interaction.author if hasattr(ctx_or_interaction, 'author') else ctx_or_interaction.user):
            message = "❌ You do not have permission to use this command."
            if hasattr(ctx_or_interaction, 'send'):
                await ctx_or_interaction.send(message, delete_after=5)
        ...
        if hasattr(ctx_or_interaction, 'send'):
            ...
            await ctx_or_interaction.response.send_message(embed=embed, view=view, ephemeral=True)
        modpanel_logger.info(f"Mod tools opened by {ctx_or_interaction.author if hasattr(ctx_or_interaction, 'author') else ctx_or_interaction.user}")
            ...
            if data:
                if action == "warnings":
                    ...
                    for uid, wdata in top_users:
                        ...
                        latest_warning = wdata['warnings'][-1] if wdata['warnings'] else {"reason": "No reason", "mod": "Unknown", "timestamp": "Unknown"}
                    ...
                    for uid, adata in top_users:
                        count = adata['count']
                        latest_action = adata['actions'][-1] if adata['actions'] else {"reason": "No reason", "mod": "Unknown", "timestamp": "Unknown"}
                        reason = latest_action['reason']
                        text_lines.append(f"<@{uid}>: {count} {action}(s) - Latest: {reason}")
                    text = "\n".join(text_lines)
            else:
                text = "No actions recorded."
            embed.add_field(name=f"{action.capitalize()}", value=text or "No actions recorded.", inline=True)  # Use inline=True for side-by-side layout
        set_pink_footer(embed, bot=self.bot.user)
        if hasattr(ctx_or_interaction, 'send'):
    ...
    async def handle_moddetails(self, ctx_or_interaction, user: discord.User):
        if not is_mod_or_admin(ctx_or_interaction.author if hasattr(ctx_or_interaction, 'author') else ctx_or_interaction.user):
            message = "❌ You do not have permission to use this command."
            if hasattr(ctx_or_interaction, 'send'):
                await ctx_or_interaction.send(message, delete_after=5)
        ...
        for action in actions:
            ...
            if data and data.get("count", 0) > 0:
                if action == "warnings":
                    ...
                    text = "\n".join(text_lines)
                embed.add_field(name=f"{action.capitalize()} ({data['count']})", value=text, inline=False)
            else:
                embed.add_field(name=f"{action.capitalize()}", value="No actions recorded.", inline=False)
        set_pink_footer(embed, bot=self.bot.user)
        if hasattr(ctx_or_interaction, 'send'):
            await ctx_or_interaction.send(embed=embed)
        else:
            await ctx_or_interaction.response.send_message(embed=embed, ephemeral=True)
        modpanel_logger.info(f"Mod details requested for {user} by {ctx_or_interaction.author if hasattr(ctx_or_interaction, 'author') else ctx_or_interaction.user}")

    @commands.command(name="moddetails")
    async def moddetails_command(self, ctx, user: discord.User):
        """
    ...
    @app_commands.command(name="moddetails", description="📊 Shows detailed moderation actions for a specific user.")
    @app_commands.describe(user="The user to get details for")
    @app_commands.guilds(discord.Object(id=int(os.getenv("DISCORD_GUILD_ID"))))
    async def moddetails_slash(self, interaction: discord.Interaction, user: discord.User):
        await self.handle_moddetails(interaction, user)

    # !optins (Prefix) - For mods/admins to view opt-ins
    @commands.command(name="optins")
    async def optins_command(self, ctx):
        """
    ...
    @app_commands.command(name="optins", description="📊 Shows an overview of user opt-ins (e.g., changelog notifications).")
    @app_commands.guilds(discord.Object(id=int(os.getenv("DISCORD_GUILD_ID"))))
    async def optins_slash(self, interaction: discord.Interaction):
        await self.handle_optins(interaction)

    # 🧩 Shared handler for optins logic
    async def handle_optins(self, ctx_or_interaction):
        if not is_mod_or_admin(ctx_or_interaction.author if hasattr(ctx_or_interaction, 'author') else ctx_or_interaction.user):
            message = "❌ You do not have permission to use this command."
            if hasattr(ctx_or_interaction, 'send'):
                await ctx_or_interaction.send(message, delete_after=5)
        ...
        if hasattr(ctx_or_interaction, 'send'):
            ...
            await ctx_or_interaction.response.send_message(embed=embed, ephemeral=True)
        modpanel_logger.info(f"Opt-ins overview requested by {ctx_or_interaction.author if hasattr(ctx_or_interaction, 'author') else ctx_or_interaction.user}")
