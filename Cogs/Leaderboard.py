...
class Leaderboard(commands.Cog):
    ...
    def create_leaderboard_embed(self, title, data_list, value_formatter=lambda x: x):
        embed = discord.Embed(
            title=f"🏆 {title}",
            color=PINK
        )
        if not data_list:
            ...
            text = "\n".join([f"{i+1}. <@{uid}>: {value_formatter(val)}" for i, (uid, val) in enumerate(data_list[:10])])
            embed.add_field(name="Top 10", value=text, inline=False)
        set_pink_footer(embed, bot=self.bot.user)
        return embed

    # 🧩 Shared handler for leaderboard logic
    async def handle_leaderboard(self, ctx_or_interaction, category):
        if category == "rl_overall":
            ...
            embed = self.create_leaderboard_embed("Highest RL Ranks Overall", sorted_data, lambda idx: f"{RANK_EMOJIS.get(RL_TIER_ORDER[idx], '<:unranked:1425389712276721725>')} {RL_TIER_ORDER[idx]}")
            ...
            sorted_data = sorted(data.items(), key=lambda x: x[1], reverse=True)
            embed = self.create_leaderboard_embed(f"Highest RL Ranks {playlist.upper()}", sorted_data, lambda idx: f"{RANK_EMOJIS.get(RL_TIER_ORDER[idx], '<:unranked:1425389712276721725>')} {RL_TIER_ORDER[idx]}")
            ...
            embed = discord.Embed(title="❌ Invalid Category", description="Use: rl_overall, rl_1v1, rl_2v2, rl_3v3, rl_4v4, tickets, messages, images", color=PINK)
            set_pink_footer(embed, bot=self.bot.user)
    ...
    async def leaderboard_command(self, ctx, category: str):
        """
    ...
    async def leaderboard_slash(self, interaction: discord.Interaction, category: str):
