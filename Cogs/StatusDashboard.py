    ...
    async def _create_status_embed(self) -> discord.Embed:
        ...
        if utility_cog and hasattr(utility_cog, "create_status_embed"):
            embed = utility_cog.create_status_embed(
                self.bot.user, self.bot.latency, len(self.bot.guilds), monitoring_data
            )
        else:
    ...
    async def setup_persistent_status(self):
        ...
        if self.status_message_id:
            try:
                ...
                logger.warning(
                    f"📊 [StatusDashboard] Saved message {self.status_message_id} not found, will create new one"
                )
                self.status_message_id = None
        ...
        logger.info(f"📊 [StatusDashboard] Background update task running (interval: {update_interval} minutes)")
