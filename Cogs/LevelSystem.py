...
class LevelSystem(commands.Cog):
    ...
    def _init_database(self):
        ...
        try:
            ...
            # Migration: Add meme post tracking columns (12. Dez 2025)
            try:
                cursor.execute("ALTER TABLE user_xp ADD COLUMN memes_posted INTEGER DEFAULT 0")
                logger.info("⭐ [LevelSystem] Added memes_posted column")
            except sqlite3.OperationalError:
                pass  # Column already exists
    ...
    def _check_meme_fetch_cooldown(self, user_id: str) -> bool:
        """
        Check if user can gain XP for meme fetch (30s cooldown)

        Returns:
            bool: True if cooldown expired or no previous fetch, False otherwise
        """
        if user_id not in self._meme_fetch_cooldowns:
            return True

        last_fetch = self._meme_fetch_cooldowns[user_id]
        cooldown = XP_CONFIG.get("meme_fetch_cooldown", 30)

        if (datetime.now(timezone.utc) - last_fetch).total_seconds() >= cooldown:
            return True

        return False

    def _update_meme_fetch_cooldown(self, user_id: str):
    ...
    async def add_xp(self, user_id: str, username: str, xp_type: str, amount: int = None) -> Optional[dict]:
        ...
        try:
            ...
            return {"xp_gained": xp_amount, "total_xp": new_xp, "level": new_level, "leveled_up": new_level > old_level}
    ...
    def _update_user_xp(self, user_id: str, xp_type: str, xp_amount: int, new_xp: int, new_level: int):
        ...
        try:
            ...
            column_mapping = {
                ...
                # New rebalanced names (12. Dez 2025)
                "meme_fetch": "memes_fetched",
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            # XP for message
            await self.add_xp(user_id=str(message.author.id), username=message.author.name, xp_type="message_sent")
            ...
            if message.attachments:
                ...
                if has_image:
                    await self.add_xp(
                        user_id=str(message.author.id), username=message.author.name, xp_type="image_sent"
                    )
        except Exception as e:
    ...
    async def handle_promote_mod(self, ctx_or_interaction, user: discord.Member):
        ...
        try:
            ...
            # Save to database
            self._save_mod_promotion(user_id=str(user.id), promoted_by=str(author.id))
    ...
    def _create_mod_promotion_embed(self, user: discord.Member, promoted_by: discord.Member) -> discord.Embed:
        ...
        embed = discord.Embed(
            ...
            description=(f"**{user.mention}** has been promoted to **Moderator**!\n\nWelcome to the team! 🎉"),
            color=0xFFD700,  # Gold
        ...
        # Fields
        embed.add_field(name="👤 New Moderator", value=user.mention, inline=True)

        embed.add_field(name="🛡️ Promoted by", value=promoted_by.mention, inline=True)

        embed.add_field(name="📅 Date", value=f"<t:{int(datetime.now(timezone.utc).timestamp())}:F>", inline=False)

        # Footer
        embed.set_footer(text="Good luck on the team!", icon_url=user.guild.icon.url if user.guild.icon else None)
    ...
    async def handle_add_xp(self, ctx_or_interaction, user: discord.Member, amount: int, xp_type: str = "manual_bonus"):
        ...
        try:
            ...
            # Add XP
            result = await self.add_xp(user_id=str(user.id), username=user.name, xp_type=xp_type, amount=amount)
    ...
    async def add_xp_prefix(
        self, ctx: commands.Context, user: discord.Member, amount: int, xp_type: str = "manual_bonus"
    ):
    ...
    @app_commands.describe(
        user="The user to add XP to", amount="Amount of XP to add", xp_type="XP type (default: manual_bonus)"
    )
    async def add_xp_slash(
        self, interaction: discord.Interaction, user: discord.Member, amount: int, xp_type: str = "manual_bonus"
    ):
