...
class RoleInfo(commands.Cog):
    ...
    def create_roleinfo_embed(self, role: discord.Role) -> discord.Embed:
        ...
        description = ROLE_DESCRIPTIONS.get(role.id, f"**{role.name}**\nNo specific description available.\n**Role ID:** `{role.id}`")
        embed = discord.Embed(
    ...
    async def roleinfo_prefix(self, ctx, *, role_name: str = None):
        """
    ...
    @app_commands.command(name="roleinfo", description="📢 Shows info, permissions, and usage tips for a given role.")
    ...
    async def roleinfo_slash(self, interaction: discord.Interaction, role_id: str = None):
        """
