...
def like_post(post_id, user_id, user_name):
    ...
    try:
        response = requests.post(f"{API_BASE_URL}/api/community_posts/{post_id}/like", headers=headers, timeout=5)
...
def test_concurrent_likes(num_users=10):
    ...
    with ThreadPoolExecutor(max_workers=num_users) as executor:
        futures = [executor.submit(like_post, post_id, user_id, f"User{user_id}") for user_id in user_ids]
...
def test_concurrent_like_unlike(num_operations=20):
    ...
    with ThreadPoolExecutor(max_workers=num_operations) as executor:
        futures = [executor.submit(like_post, post_id, user_id, f"User{user_id}") for _ in range(num_operations)]
