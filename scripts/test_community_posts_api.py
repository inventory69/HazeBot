...
def test_create_post(token, content=None, image=None, is_announcement=False):
    ...
    print(f"Request: POST /api/posts")
...
def main():
    ...
    post_id_1 = test_create_post(
        token, 
        content="🧪 Test Post #1 - Text only content", 
        is_announcement=False
    )
    
    # Step 4: Test CREATE - Text + Image
    test_image = create_test_image()
    post_id_2 = test_create_post(
        token, 
        content="🧪 Test Post #2 - With image", 
        image=test_image,
        is_announcement=False
    )
    
    # Step 5: Test CREATE - Announcement (admin only)
    post_id_3 = test_create_post(
        token, 
        content="📢 Test Announcement - Admin only", 
        is_announcement=True
    )
    
    # Step 6: Test GET - Retrieve all posts
    posts = test_get_posts(token, limit=10, offset=0)
